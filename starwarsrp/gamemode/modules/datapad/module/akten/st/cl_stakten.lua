...
local function createAkte(pnl)
    ...
    local btn = PD.Button("Akte Erstellen ", pnl, function()
        ...
        PD.DataPad.SendDataToServer("Stoßtruppen", data)
    ...
    local btn = PD.Button("Zurück", pnl, function()
        ...
        PD.DataPad:Menu({"Akten", "Stoßtruppen"})
    end)
    btn:Dock(TOP)
    btn:SetTall(PD.H(50))
    btn:SetRadius(20)
...
local function STMenu(pnl)
    pnl:Clear()

    local scrl = PD.Scroll(pnl)

    for k, v in SortedPairs(PD.DataPad.GetData("Stoßtruppen")) do
...
PD.DataPad.AddEntry("Akten", "Stoßtruppen", function(pnl)
    STMenu(pnl)
end)