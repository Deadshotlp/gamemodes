...
local function createAkte(pnl)
    ...
    local btn = PD.Button("Akte Erstellen ", pnl, function()
        ...
        PD.DataPad.SendDataToServer("Technische Akten", data)
    ...
    local btn = PD.Button("Zurück", pnl, function()
        ...
        PD.DataPad:Menu({"Akten", "Technische Akten"})
    end)
    btn:Dock(TOP)
    btn:SetTall(PD.H(50))
    btn:SetRadius(20)
...
local function MedicMenu(pnl)
    ...
    for k, v in SortedPairs(PD.DataPad.GetData("Technische Akten")) do
...
PD.DataPad.AddEntry("Akten", "Technische Akten", function(pnl)
    MedicMenu(pnl)
end)